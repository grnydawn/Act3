...
import hashlib
import subprocess
#import inspect # KGEN deletion
from funcsigs import signature # KGEN addition
from collections import defaultdict
#from dbm import dumb # KGEN deletion
#import dbm as ddbm # KGEN deletion
    ...
    def get_status(self, task, tasks_dict, get_log=False):
        ...
        for utd, utd_args, utd_kwargs in task.uptodate:
            ...
            if hasattr(utd, '__call__'):
                ...
                # if present.
                #spec_args = list(inspect.signature(utd).parameters.keys()) # KGEN deletion
                spec_args = list(signature(utd).parameters.keys()) # KGEN addition
                magic_args = []
