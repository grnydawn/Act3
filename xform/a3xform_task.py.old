    ...
    def _perform(self):
        ...
        try:
            for form, std, extra in [('fixed', 'legacy', ''), ('free', 'f95', ''), \
                ...
                stdout, stderr = runcmd('gfortran-4.8 -c -std=%s -f%s-form -o %s %s %s'%\
                ...
                logger().debug('%s is checked as %s'%(self.filepath, self.retval['filetype']))
                break
        except Exception as e:
            logger().debug('Check Exception: %s'%(str(e)))
            pass
...
class TaskFileCheckMakefile(A3Task):
    ...
    def _perform(self):
        ...
        logger().debug('make STDOUT: %s'%str(stdout))
        logger().debug('make STDERR: %s'%str(stderr))
        if stderr:
            return self.retval
        else:
            self.retval['error'] = False
...
class TaskFileCheckExt(A3Task):
    ...
    def get_checker(self, ext):
        for exts, checker in self.checkmap.items():
            if ext in exts:
                logger().debug('A checker for %s is found'%ext)
                return checker
        return None
    ...
    def _perform(self):
        if not os.path.exists(self.abspath):
            ...
            logger().debug('%s does not exist.'%self.basename)
            return self.retval
        ...
        if self.has_ext(self.fileext):
            ...
            if not task1_retval['error']:
                ...
                logger().debug('%s is checked as %s'%(self.filepath, task1_retval['filetype']))
        else:
            self.retval['error'] = True
            self.pack_error('%s extension is not supported.'%self.fileext)
            logger().debug('%s extension is not supported.'%self.fileext)
    ...
    def _perform(self):
        ...
        if self.has_filename(self.basename):
            ...
            if not task_retval['error']:
                ...
                logger().debug('%s is checked as %s'%(self.filepath, task_retval['filetype']))
        else:
            self.retval['error'] = True
            self.pack_error('%s is not supported.'%self.basename)
            logger().debug('%s is not supported.'%self.basename)
...
class TaskFileCheck(A3Task):
    ...
    def _perform(self):
        ...
        logger().debug('File check begins for %s'%self.filepath)
        ...
        if not task_retval['error']:
            self.retval['error'] = False
            self.retval['filetype'] = task_retval['filetype']
            logger().debug('%s is checked as %s based on extension'%(self.filepath, task_retval['filetype']))
            return self.retval
        ...
        if not task_retval['error']:
            self.retval['error'] = False
            self.retval['filetype'] = task_retval['filetype']
            logger().debug('%s is checked as %s based on filename'%(self.filepath, task_retval['filetype']))
            return self.retval
